...
def make_train(config):
    ...
    basic_env = CraftaxSymbolicEnv()
    DEFAULT_STATICS = basic_env.default_static_params()
    env_params = basic_env.default_params
    log_env = LogWrapper(basic_env)
    dist_env = DistResetEnvWrapper(log_env)
    env = BatchEnvWrapper(dist_env, num_envs=config["NUM_ENVS"])
    ...
    sample_random_level = lambda rng: generate_world(rng, basic_env.default_params, DEFAULT_STATICS)
    ...
    def train(rng):
        ...
        def create_agent(rng):
            ...
            tx = optax.chain(
                ...
                ti_ada(vy0 = jnp.zeros(config["BUFFER_SIZE"]), eta=lr)
            )
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env(carry, _):
                ...
                new_obs, new_env_state, reward, new_done, info = jax.vmap(dist_env.step_with_dist_reset, in_axes=(0, 0, 0, None, None, None))(
                    jax.random.split(rng_s, config["NUM_ENVS"]), env_state, new_action, train_state.y, train_state.levels, env_params
                )
            ...
            def score_fn(train_state, rng):
                ...
                def _greedy_env_step(step_state, _):
                    ...
                    eps = jnp.full(config["BUFFER_SIZE"], config["EPS_TEST"])
                    new_action = jax.vmap(eps_greedy_exploration)(
                        jax.random.split(rng_a, config["BUFFER_SIZE"]), q_vals, eps
                    )
                    new_obs, new_env_state, reward, new_done, info = score_env.step(
                ...
                init_action = jnp.zeros((config["BUFFER_SIZE"]), dtype=int)
        ...
        def get_test_metrics(train_state, rng):
            ...
            num_levels = config['TEST_NUM_ENVS']
            levels = jax.vmap(generate_world, (0, None, None))(jax.random.split(jax.random.PRNGKey(101), num_levels), env_params, DEFAULT_STATICS)
            init_obs, env_state = jax.vmap(test_env.reset_to_level, (0, 0, None))(jax.random.split(rng_reset, num_levels), levels, env_params)
        ...
        rng, _rng = jax.random.split(rng)
        obs, env_state = jax.vmap(dist_env.reset_env_to_level, in_axes=(0, 0, None))(jax.random.split(_rng, config["NUM_ENVS"]), levels, env_params)
