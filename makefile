...
DOCKER_IMAGE_NAME = pqn_nmonette
...
# make file commands
build:
	DOCKER_BUILDKIT=1 docker build --build-arg USE_CUDA=$(USE_CUDA) --build-arg UID=$(ID) --build-arg GID=1234 --build-arg REQS="$(shell cat ./requirements/requirements.txt | tr '\n' ' ')" --tag $(IMAGE) --progress=plain ${PWD}/.
